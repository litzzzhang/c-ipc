// simpleBVH from https://github.com/geometryprocessing/SimpleBVH
...
namespace SimpleBVH {
using VectorMax3d = Eigen::Matrix<double, Eigen::Dynamic, 1, Eigen::ColMajor, 3, 1>;

static bool box_box_intersection(
    const Eigen::Vector3d &min1, const Eigen::Vector3d &max1, const Eigen::Vector3d &min2,
...
class BVH {
  public:
    ...
    void intersect_3D_box(
        ...
        for (int i = 0; i < tmp.size(); ++i) list[i] = new2old[tmp[i]];
    }

    void intersect_2D_box(
        const Eigen::Vector2d &bbd0, const Eigen::Vector2d &bbd1,
        std::vector<unsigned int> &list) const {
    ...
    void intersect_box(
        const VectorMax3d &bbd0, const VectorMax3d &bbd1, std::vector<unsigned int> &list) const {
        Eigen::Vector3d bbd0_3D = Eigen::Vector3d::Zero();
  ...
  private:
    void init_boxes_recursive(
        const std::vector<std::array<Eigen::Vector3d, 2>> &cornerlist, int node_index, int b,
        int e);

    void box_search_recursive(
        const Eigen::Vector3d &bbd0, const Eigen::Vector3d &bbd1, std::vector<unsigned int> &list,
        int n, int b, int e) const;

    bool
    box_intersects_box(const Eigen::Vector3d &bbd0, const Eigen::Vector3d &bbd1, int index) const;
...
inline void BVH::init_boxes_recursive(
    const std::vector<std::array<Eigen::Vector3d, 2>> &cornerlist, int node_index, int b, int e) {
    assert(b != e);
...
inline void BVH::box_search_recursive(
    const Eigen::Vector3d &bbd0, const Eigen::Vector3d &bbd1, std::vector<unsigned int> &list,
    int n, int b, int e) const {
    assert(e != b);
...
inline int BVH::max_node_index(int node_index, int b, int e) {
    assert(e > b);
    if (b + 1 == e) { return node_index; }
    int m = b + (e - b) / 2;
    int childl = 2 * node_index;
    int childr = 2 * node_index + 1;
...
inline void BVH::init(const std::vector<std::array<Eigen::Vector3d, 2>> &cornerlist) {
    n_corners = static_cast<integer>(cornerlist.size());
    ...
    if (scale > 100) { box_centers /= scale; }
    ...
    for (int i = 0; i < n_corners; i++) {
        ...
        list[i].morton = MortonCode64(int(tmp(0)), int(tmp(1)), int(tmp(2)));
        list[i].order = i;
    }
    ...
    for (int i = 0; i < n_corners; i++) { new2old[i] = list[i].order; }

    std::vector<std::array<Eigen::Vector3d, 2>> sorted_cornerlist(n_corners);

    for (int i = 0; i < n_corners; i++) { sorted_cornerlist[i] = cornerlist[list[i].order]; }

    boxlist.resize(
        max_node_index(1, 0, n_corners) + 1 // <-- this is because size == max_index + 1 !!!
    );

    init_boxes_recursive(sorted_cornerlist, 1, 0, n_corners);
}

inline void BVH::init(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F, const double tol) {
    assert(F.cols() == 3);
    assert(V.cols() == 3);
...
inline bool
BVH::box_intersects_box(const Eigen::Vector3d &bbd0, const Eigen::Vector3d &bbd1, int index) const {
