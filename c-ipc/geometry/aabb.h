...
inline bool AABB::is_intersect(const AABB &other) const {
...
inline void AABB::conservative_inflation(const double inflation_radius) {
...
inline void AABB::build_vertex_boxes(
    const Matrix3Xr &vertices, std::vector<AABB> &vertex_boxes, const double inflation_radius) {
...
inline void AABB::build_edge_boxes(
    const std::vector<AABB> &vertex_boxes, const Matrix2Xi &edges, std::vector<AABB> &edge_boxes) {
    ...
    for (integer i = 0; i < edge_num; i++) {
        ...
        aabb.vertex_ids = {edges(i, 0), edges(i, 1), -1};
        edge_boxes[i] = aabb;
    }
}

inline void AABB::build_face_boxes(
    const std::vector<AABB> &vertex_boxes, const Matrix3Xi &faces, std::vector<AABB> &face_boxes) {
    integer face_num = static_cast<integer>(faces.cols());
    face_boxes.resize(face_num);
    for (integer i = 0; i < face_num; i++) {
        AABB aabb(vertex_boxes[faces(0, i)], vertex_boxes[faces(1, i)], vertex_boxes[faces(2, i)]);
